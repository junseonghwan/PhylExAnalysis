...
parser = argparse.ArgumentParser(description='Script for running single-region + scRNA seq simulation studies.')
...
parser.add_argument('--region_count', help="Number of regions to run.", type=int, default = 3)
...
for rep_no in range(REP_BEGIN, REP_END):
    ...
    for case_no in range(CASE_BEGIN, CASE_END):
        ...
        # Generate single region data files.
        run_command = "./ProcessSingleRegionData.sh " + CASE_PATH
        os.system(run_command)

        for region in range(1, args.region_count + 1):
            # write main.config file to REP_PATH
            for chain in range(args.nchains):
                seed = np.random.randint(100000000)
                rep_bulk_path = CASE_PATH + "/single_region_genotype_ssm" + str(region) + ".txt"
                output_path = CASE_PATH + "/single_region_genotype" + str(region) + "/"
